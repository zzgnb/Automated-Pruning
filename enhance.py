...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, index) -> Tuple[str, Tensor, int]:  # [filename, audio tensor(1, T), sr]
        fn = self.files[index]
        # note that self.sr is the resample rate fitting to the model, but meta.sample_rate is original sr
...
def main(args):
    ...
    for i, (file, audio, audio_sr) in enumerate(loader):
        ...
        progress_percent = (i + 1) / n_samples * 100
        t0 = time.time()
        audio = enhance(
        ...
        p_str = f"{progress_percent:2.0f}% | " if n_samples > 1 else ""
...
def init_df(
    ...
    Returns:
        ...
        df_state (DF): Deep filtering state for stft/istft/erb, state means hparameters w.r.t. the above.
        suffix (str): Suffix based on the model name. This can be used for saving the enhanced audio.
    """
    try:
    ...
    # download model from github, but packet loss always exists so prepared already
    # model_base_dir = get_model_basedir(model_base_dir or default_model)
    # model_base_dir = '/E/zhaozugang/SpeechEhancement/DeepFilterNet/self_test/DeepFilterNet2'
...
# generate features from audio, composed of nb_energy and complex spectrum
...
def enhance(
    ...
    # mix the enhanced and noisy audio to meet the atten_lim
...
def setup_df_argument_parser(default_log_level: str = "INFO") -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "-m",
        type=str,
        default="/E/zhaozugang/SpeechEhancement/DeepFilterNet/self_test/train_result/48KHz/tmp/DFNet2+RS_add_sf_sgru",
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="/E/zhaozugang/Dataset/VoiceBank_Demand_test/48KHz/DFnet2_new",
...
def run():
    ...
    parser.add_argument(
        ...
        type=str,
        # default=None,
        default="/E/zhaozugang/Dataset/VoiceBank_Demand_test/48KHz/noisy",
...
if __name__ == "__main__":
    # enhance.py can accept any sample rate audio because of the inherent restorability of the progress
    run()
    # TODO: change the model_base_dir, output_dir, noisy_dir
