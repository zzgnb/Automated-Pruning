...
class DfParams:
    def __init__(self, sr):
        ...
        self.sr: int = config("SR", cast=int, default=sr, section="DF")
        if sr == 48000:
            ...
            # sr = 16_000
            # erb_widths: [ 1 1 1 1 1 1 2 2 | 2 3 3 4 4 5 5 7 | 7 8 10 12 | 13 15 18 20 | 24 28 31 37 42 50 56 67]
            # 5KHz corresponds to 96 bins
            # [1, 16] = 43 bin, [17, 24] = 103 bin, [25, 32] = 337 bin, 16KHz need 24 bands
            self.fft_size: int = config("FFT_SIZE", cast=int, default=320, section="DF")
            self.hop_size: int = config("HOP_SIZE", cast=int, default=160, section="DF")
            self.nb_erb: int = config("NB_ERB", cast=int, default=24, section="DF")
            self.nb_df: int = config("NB_DF", cast=int, default=96, section="DF")
        ...
        self.df_lookahead: int = config("DF_LOOKAHEAD", cast=int, default=2, section="DF")
...
class Config:
    ...
    def __call__(self, option: str, default: Any = None, cast: Type[T] = str, save: bool = True, section: Optional[str] = None) -> T:
