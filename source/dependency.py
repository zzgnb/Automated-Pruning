...
class Group(object):
    ...
    def prune(self, idxs=None, record_history=True):
        ...
        if record_history:
            ...
            self._DG._pruning_history.append([root_module, self._DG.is_out_channel_pruning_fn(pruning_fn), root_pruning_idx])
...
class DependencyGraph(object):
    ...
    def pruning_history(self) -> typing.List[typing.Tuple[nn.Module, bool, typing.Union[list, tuple]]]:
    ...
    def load_pruning_history(self, pruning_history):
        ...
        for module, is_out_channel_pruning, pruning_idx in self._pruning_history:
    ...
    def build_dependency(
        ...
        if isinstance(example_inputs, dict):
            for p in example_inputs.values():
                self.device = p.device.type.lower()
                break
        elif isinstance(example_inputs, tuple):
            self.device = example_inputs[0].device.type.lower()
        else:
            self.device = example_inputs.device.type.lower()
        ...
        # Build computational graph through tracing(DFS). module2node is the computational graph
        self.module2node = self._trace(model, example_inputs, forward_fn, output_transform=output_transform)
    ...
    def get_pruning_group(
        ...
        # add dependency for root node itself
    ...
    def get_all_groups(self, ignored_layers=[], root_module_types=(ops.TORCH_LINEAR)):
        visited_layers = []
        ignored_layers = ignored_layers + self.IGNORED_LAYERS
        for m in list(self.module2node.keys()):
            # pruning RNN will only treat hh as root, excluding ih
            # ih will be pruned if set in root_module_types
            if isinstance(m, ops.RNN_hh):
                if (m.rnn_layer in ignored_layers 
                    or not isinstance(m.rnn_layer, tuple(root_module_types))
                    ):
                    continue
            else:
                if (m in ignored_layers 
                    or not isinstance(m, tuple(root_module_types))
                    ):
                    continue
    ...
    def _build_dependency(self, module2node):
        ...
        for module, node in module2node.items():
            ...
            for in_node in node.inputs:
                ...
                dep = Dependency(trigger=trigger, handler=handler, source=node, target=in_node)
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        def _record_grad_fn(module, inputs, outputs):
            ...
            if isinstance(module, nn.LSTM) and self.device == "cpu":
                # from outputs to seek for ih and hh addmmbackward of LSTM
                suqeeze_bw = outputs.grad_fn
                stack_bw = suqeeze_bw.next_functions[0][0]
                mul_bw = stack_bw.next_functions[0][0]
                sigmoid_bw = mul_bw.next_functions[0][0]
                unsafesplit_bw = sigmoid_bw.next_functions[0][0]
                add_bw = unsafesplit_bw.next_functions[0][0]
                
                addmm_bw_hh = add_bw.next_functions[0][0]
                gradfn2module[addmm_bw_hh] = ops.RNN_hh(module, addmm_bw_hh)

                unbind_bw = add_bw.next_functions[1][0]
                view_bw = unbind_bw.next_functions[0][0]
                addmm_bw_ih = view_bw.next_functions[0][0]
                gradfn2module[addmm_bw_ih] = ops.RNN_ih(module, addmm_bw_ih)
                return

            if isinstance(module, nn.GRU) and self.device == "cpu":
                # from outputs to seek for ih and hh addmmbackward of GRU
                suqeeze_bw = outputs.grad_fn
                stack_bw = suqeeze_bw.next_functions[0][0]
                add0_bw = stack_bw.next_functions[0][0]

                mul_bw = add0_bw.next_functions[0][0]
                sigmoid_bw = mul_bw.next_functions[1][0]
                add1_bw = sigmoid_bw.next_functions[0][0]
                unsafesplit0_bw = add1_bw.next_functions[0][0]
                unsafesplit1_bw = add1_bw.next_functions[1][0]
                
                addmm_bw_hh = unsafesplit0_bw.next_functions[0][0]
                gradfn2module[addmm_bw_hh] = ops.RNN_hh(module, addmm_bw_hh)

                unbind_bw = unsafesplit1_bw.next_functions[0][0]
                view_bw = unbind_bw.next_functions[0][0]
                addmm_bw_ih = view_bw.next_functions[0][0]
                gradfn2module[addmm_bw_ih] = ops.RNN_ih(module, addmm_bw_ih)
                return
            
            # gradfn2module only store nn.module with corresponding grad_fn up to here, 
            # excluding ops such as spilt but the only exception RNN_ih/hh
    ...
    def _trace_computational_graph(self, module2node, grad_fn_root, gradfn2module, reused, visited=set()):
        ...
        def create_node_if_not_exists(grad_fn):
            ...
            if module not in module2node:
                if isinstance(module, (ops.RNN_hh, ops.RNN_ih)):
                    module_name = self._module2name.get(module.rnn_layer, None)
                else:
                    module_name = self._module2name.get(module, None)
                node = Node(module=module, grad_fn=grad_fn, name=module_name)
        ...
        while len(processing_stack) > 0:
            ...
            if hasattr(grad_fn, "next_functions"):
                for f in grad_fn.next_functions:
                    if f[0] is not None:
                        # the leaf variable as the ending
    ...
    def _update_concat_index_mapping(self, cat_node: Node):
        ...
        for ch in chs:
            if ch is None: 
                ...
                cat_node.module.concat_sizes = None
                return
            offsets.append(offsets[-1] + ch)
    ...
    def infer_channels_between(self, node_1, cat_node):
