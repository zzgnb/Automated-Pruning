...
class MultiFrameModule(nn.Module):
    ...
    def spec_unfold(self, spec: Tensor):
        ...
        """
        if self.need_unfold:
            return self.pad(spec).unfold(2, self.frame_size, 1)  #unfold: dim, window_size, stride
        return spec.unsqueeze(-1)
...
class DF(MultiFrameModule):
    ...
    def forward(self, spec: Tensor, coefs: Tensor):
        ...
        coefs = coefs.view(coefs.shape[0], -1, self.frame_size, coefs.shape[-2], coefs.shape[-1])
        if self.conj:
            coefs = coefs.conj()
...
class MfHybird(MultiFrameModule):
    """MF-MVDR mixed with MF-WF based on Rxx**-1 and speech IFC vector."""
    ...
    def forward(self, spec: Tensor, lsnr: Tensor, ifc: Tensor, iRxx: Tensor) -> Tensor:
        """MF-Hybird filter based on Rxx**-1 and speech IFC vector.
        ...
        if not self.inverse:  # Is already an inverse input. No need to inverse it again.
            ...
            iRxx = _tik_reg(iRxx, self.dload, self.eps)
            # MVDR
            numerator = torch.linalg.solve(iRxx, ifc)
            # WF
            w_WF = torch.linalg.solve(iRxx, ifc).unsqueeze(1)
        else:  # We already have an inverse estimate. Just compute the linear combination.
            numerator = torch.einsum("...nm,...m->...n", iRxx, ifc)  # [B, C, F, N]
            w_WF = torch.einsum("...nm,...m->...n", iRxx, ifc).unsqueeze(1)  # [B, 1, T, F, N]
        denumerator = torch.einsum("...n,...n->...", ifc.conj(), numerator)
        # Normalize numerator
        scale = ifc[..., -1, None].conj()
        w_MVDR = (numerator * scale / (denumerator.real.unsqueeze(-1) + self.eps)).unsqueeze(1)

        assert self.mode in ["ALPHA", "SNR"], "MF-Hybird mode must be ALPHA or SNR"
        if self.mode == "ALPHA":
            w = self.alpha * w_WF + (1-self.alpha) * w_MVDR
        else:
            # lsnr: [B,T,1]->[B,T]  spec_f, w: [B,1,T,F,N]->[B,T,F,N]
            lsnr = lsnr.squeeze(-1)
            w_WF = w_WF.squeeze(1)
            w_MVDR = w_MVDR.squeeze(1)
            w = torch.zeros_like(w_WF)
            # print(f"w_WF shape: {w_WF.shape} | type: {w_WF.type}")
            # print(f"w_MVDR.shape: {w_MVDR.shape} | type: {w_MVDR.type}")
            # print(f"LSNR: {lsnr.shape} | type: {lsnr.type}")  
            for b in range(lsnr.shape[0]):
                for t in range(lsnr.shape[1]):
                    w[b, t, :, :] = w_WF[b, t, :, :] if lsnr[b][t] > self.SNR_thres else w_MVDR[b, t, :, :]
            w = w.unsqueeze(1)
