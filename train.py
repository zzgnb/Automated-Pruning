...
@logger.catch
def main():
    global should_stop, state, debug, log_timings
    cosine_decay = True

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--data_config_file", 
        default=f"/E/zhaozugang/SpeechEhancement/DeepFilterNet/self_test/small_hdf5/{sr}KHz/dataset.cfg", 
        type=str, 
        help="Path to a dataset config file.")
    parser.add_argument(
        "--data_dir", 
        default=f"/E/zhaozugang/SpeechEhancement/DeepFilterNet/self_test/small_hdf5/{sr}KHz", 
        type=str,   
        help="Path to the dataset directory containing .hdf5 files."
    )
    parser.add_argument(
        "--base_dir", 
        default=f"/E/zhaozugang/SpeechEhancement/DeepFilterNet/self_test/train_result/{sr}KHz/" + "tmp/DFNet2_jit_S_Lin_s",
        type=str, 
        help="Directory to store logs, summaries, checkpoints, etc."
    )
    parser.add_argument(
    ...
    bs: int = config("BATCH_SIZE", 96, int, section="train")
    ...
    dataloader = DataLoader(
        ...
        snrs=config("DATALOADER_SNRS", [-10, -5, 0, 5, 10, 20, 30], Csv(int), section="train"),  # type: ignore
        log_level=log_level,
    )
    ...
    batch_size_scheduling: List[str] = config("BATCH_SIZE_SCHEDULING", batch_size_scheduling, Csv(str), section="train")  # type: ignore
    scheduling_bs = bs
    ...
    if len(batch_size_scheduling) > 0:
        ...
        logger.info(f"batch_size_scheduling: {batch_size_scheduling}")
    ...
    wds = setup_wds(len(dataloader), cosine_decay)
    ...
    # if config.get("model", str, "train") != "deepfilternetmf" or config.get("mfop_method", str, "deepfilternet") != "HYBRID" or \
    #     config.get("hybird_mode", str, "deepfilternet") != "SNR":
    #     lsnrl_f = 0
    losses = setup_losses(model=model, sl=sl_f, mrsl=mrsl_f, s1l=s1l_f, s2l=s2l_f, gs2l=gs2l_f)

    if config("START_EVAL", True, cast=bool, section="train"):
        val_loss = run_epoch(
            model=model,
            epoch=epoch - 1,
            loader=dataloader,
        ...
        with set_detect_anomaly(detect_anomaly and is_train), set_grad_enabled(is_train):
            ...
            if is_train:
                try:
                    ...
                    # clip_grad_value_(model.parameters(), 3)
...
def setup_losses(model, sl, mrsl, s1l, s2l, gs2l) -> Loss:
    global state, istft
    assert state is not None

    p = ModelParams()
    istft = Istft(p.fft_size, p.hop_size, torch.as_tensor(state.fft_window().copy())).to(get_device())
    loss = Loss(state=state, istft=istft, model=model, 
                sl_f=sl, mrsl_f=mrsl, s1l_f=s1l, s2l_f=s2l, gs2l_f=gs2l).to(get_device())
...
def load_opt(
    ...
    lr = config("lr", 1e-3, float, section="optim")
...
def setup_lrs(steps_per_epoch: int) -> np.ndarray:
    ...
    # if config.get("model", str, "train") == "deepfilternetmf":
    #     lr_epoch_decay = config("lr_decay_per_epoch", 0.5, float, section="optim")
    #     lr_values = normal_scheduler(lr, num_epochs, steps_per_epoch, lr_epoch_decay)
    # else:
...
def setup_wds(steps_per_epoch: int, cosine_decay: bool) -> Optional[np.ndarray]:
    decay = config.get("weight_decay", float, "optim")
    num_epochs = config.get("max_epochs", int, "train")

    if not cosine_decay:
        decay_values = np.full((num_epochs*steps_per_epoch,), fill_value=decay)
    else:
        decay_start = config("weight_decay_start", 1e-6, float, section="optim")
        if decay_start == -1:
            return None
        if decay == 0.0:
            decay = 1e-12
        ...
        decay_values = cosine_scheduler(
            decay_start, decay, niter_per_ep=steps_per_epoch, epochs=num_epochs
        )
