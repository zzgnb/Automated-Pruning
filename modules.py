...
def erb_fb(widths: np.ndarray, sr: int, normalized: bool = True, inverse: bool = False) -> Tensor:
    """
    Generate fft to Erb matrix or inverse, shape [F, erb] or [erb, F]
    """
    n_freqs = int(np.sum(widths))  # subband num
...
class Mask(nn.Module):
    """
    Convert erb gain to fft gain and add post filter to get the final mask
    Return: ERB-Gain Masked fft
    """
    ...
    def forward(self, spec: Tensor, mask: Tensor, atten_lim: Optional[Tensor] = None) -> Tensor:
        ...
        # mask (real): [B, 1, T, Erb], Erb: erb_bins
        # erb_inv_fb: [Erb, F]
        # atten_lim: [B]
...
class DfOp(nn.Module):
    ...
    def forward_real_unfold(
        ...
        # Version2: Unfold
        # spec (real) [B, 1, T, F, 2]
        # coefs (real) [B, T, O, F, 2], O: df_order
        # alpha (real) [B, T, 1]
        padded = spec_pad(
            # [B, T, df, 2] -> [B, T+O-1, df, 2] 
            spec[..., :self.df_bins, :].squeeze(1), self.df_order, self.df_lookahead, dim=-3
        )
        padded = padded.unfold(dimension=1, size=self.df_order, step=1)  # slide window along dim=T to [B, T, df, 2, O]
        padded = padded.permute(0, 1, 4, 2, 3)  # [B, T, O, df, 2]
        ...
        spec_f = spec_f.sum(dim=2, keepdim=False)  # [B, T, df, 2]
        return assign_df(spec, spec_f.unsqueeze(1), self.df_bins, alpha)

    def forward_complex_strided(
        self, spec: Tensor, coefs: Tensor, alpha: Optional[Tensor] = None
    ) -> Tensor:
        # Version3: Complex strided; definitly nicest, no permute, no indexing, but complex gradient
        # spec (real) [B, 1, T, F, 2], O: df_order
        # coefs (real) [B, T, O, F, 2]
        # alpha (real) [B, T, 1]
        padded = as_strided(
...
def spec_pad(x: Tensor, df_order: int, lookahead: int, dim: int = 0) -> Tensor:
    # x: spec[B, T, df, 2], df_order: 5
    pad = [0] * x.dim() * 2
    if dim >= 0:
        pad[(x.dim() - dim - 1) * 2] = df_order - lookahead - 1
        pad[(x.dim() - dim - 1) * 2 + 1] = lookahead
    else:
        pad[(-dim - 1) * 2] = df_order - lookahead - 1
        pad[(-dim - 1) * 2 + 1] = lookahead
    # the pad sequence will be inversed when padding, 
    # so the dim in spec_pad is exactly where to pad for input
    return F.pad(x, pad) 
...
class GroupedGRU(nn.Module):
    ...
    def flatten_parameters(self):  # 参数内存连续
...
class prGRU(nn.Module):
    ...
    def __init__(
        ...
        self.grus.append(nn.GRU(hidden_size, hidden_size, num_layers=1, batch_first=batch_first).cuda())
...
# prGRU for jit with one layer in gru
class oneGRU(nn.Module):
    input_size: Final[int]
    hidden_size: Final[int]
    def __init__(
        self,
        ...
        if lin_groups == 1:
            ...
            self.linear_in = GroupedLinear(input_size, hidden_size, lin_groups)
        self.gru = nn.GRU(hidden_size, hidden_size, num_layers=1, batch_first=batch_first).cuda()
...
# prGRU for jit with two layer in gru
class doubleGRU(nn.Module):
    ...
    def __init__(
        ...
        if lin_groups == 1:
            ...
            self.linear_in = GroupedLinear(input_size, hidden_size, lin_groups)
        self.gru1 = nn.GRU(hidden_size, hidden_size, num_layers=1, batch_first=batch_first).cuda()
        self.gru2 = nn.GRU(hidden_size, hidden_size, num_layers=1, batch_first=batch_first).cuda()
...
class GroupedLinear(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, groups: int = 1, shuffle: bool = True):
        ...
        self.input_size = input_size // groups
        self.hidden_size = hidden_size // groups
        self.gin_slice, self.gout_slice = [0], [0]
        for _ in range(groups):
            self.gin_slice.append(self.gin_slice[-1] + self.input_size)
        for _ in range(groups):
            self.gout_slice.append(self.gout_slice[-1] + self.hidden_size)
    ...
    def forward(self, x: Tensor) -> Tensor:
        outputs: List[Tensor] = []
        # print(self.gin_slice)
        for i, layer in enumerate(self.layers):
            outputs.append(layer(x[..., self.gin_slice[i] : self.gin_slice[i+1]]))
        output = torch.cat(outputs, dim=-1)
        if self.shuffle:
            orig_shape = output.shape
            if self.gout_slice[-1] % self.groups == 0:
                output = (
...
if __name__ == "__main__":
    # test_erb(model_type="deepfilternet2_16K")

    # spec (real) [B, 1, T, F, 2]
    # coefs (real) [B, T, O, F, 2], O: df_order
    # alpha (real) [B, T, 1]
    spec = torch.ones((10, 10, 10, 10))
    padded = F.pad(spec, [1, 1, 0, 0, 0, 0, 0, 0])
    # padded = spec_pad(spec, 5, 2, dim=-2)
    print(padded.shape)